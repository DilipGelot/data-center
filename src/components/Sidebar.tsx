...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Sidebar Panel - No more background overlay/blur! */}
      <div
        className={`absolute right-0 top-0 bottom-0 w-[420px] bg-white shadow-[-10px_0_30px_rgba(0,0,0,0.1)] text-slate-700 z-50 transform transition-transform duration-500 max-w-full flex flex-col pointer-events-auto ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        {/* Header - Dark Blue solid background */}
        <div className="bg-[#2f4266] text-white p-6 relative">
          <button 
            onClick={() => setSelectedId(null)}
            className="absolute top-6 right-6 p-1 hover:bg-[#1f2d48] rounded transition-colors"
          >
          ...
          <h1 className="text-2xl font-bold tracking-tight mb-2 pr-8">{data?.title}</h1>
          <p className="text-[15px] font-light text-slate-300 leading-snug pr-8">
            {data?.subtitle}
          </p>
        </div>
        ...
        <div className="p-8 flex-1 overflow-y-auto">
          ...
          <div className="space-y-6 text-[15px] leading-relaxed text-slate-600 mb-8">
          ...
          <div className="mt-8 border border-slate-200 rounded-xl p-6 bg-slate-50 shadow-sm flex flex-col items-center justify-center min-h-[160px]">
             ...
             <p className="font-bold text-slate-700">Detailed Telemetry Map</p>
             <p className="text-sm text-slate-500 mt-1">Control, Monitor, and Manage Data</p>
          </div>
